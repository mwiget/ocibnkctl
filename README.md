...
Single-binary CLI that deploys F5 BIG-IP Next for Kubernetes (BNK) 2.3.2
...
scaffold the PoC → deploy F5 BIG-IP Next 2.3.2 → inspect every pod → diagnose a
...
ocibnkctl manifest probe 2.3.3-3.2598.3-0.0.999           # a candidate tag
...
Docker Desktop given **10 CPUs and 16 GB**. The full two-node BNK 2.3.2
...
release is plain `v2.3.2`, then `v2.3.2-1`, `v2.3.2-2`, … The `2.3.2`
...
that ships a fresh binary against the same BNK release. Previous BNK
lines are frozen on branches: `release/2.3.1` (tags `v2.3.1` …
`v2.3.1-13`) and `release/2.3.0` (tags `v2.3.0`, `v2.3.0-1`).
...
F5 BIG-IP Next for Kubernetes (BNK) 2.3.2 on a **two-node k3s cluster**:
...
internal/version/      build-stamped + BNK 2.3.2 pins + min-spec floor
```
