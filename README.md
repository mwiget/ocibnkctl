...
Single-binary CLI that deploys F5 BIG-IP Next for Kubernetes (BNK) 2.4.0
...
ocibnkctl manifest probe 2.4.1                            # a candidate tag
...
release is plain `v2.4.0`, then `v2.4.0-1`, `v2.4.0-2`, … The `2.4.0`
...
F5 BIG-IP Next for Kubernetes (BNK) 2.4.0 on a **two-node k3s cluster**:
...
internal/version/      build-stamped + BNK 2.4.0 pins + min-spec floor
```
